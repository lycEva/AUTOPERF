...
""",
    ...
    def test_npu_scripts_use_pid_environment_without_docker_top(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            bin_dir = Path(tmp)
            self.write_executable(
                bin_dir / "docker",
                """#!/usr/bin/env bash
echo "docker should not be called when AUTOPERF_MONITOR_PIDS is set" >&2
exit 19
""",
            )
            self.write_executable(
                bin_dir / "npu-smi",
                """#!/usr/bin/env bash
if [ "$1" = "info" ] && [ "$2" = "-t" ] && [ "$3" = "proc-mem" ] && [ "$4" = "-i" ]; then
  echo '| 0 | 201 | python | 2048 MiB |'
elif [ "$1" = "info" ] && [ "$2" = "-t" ] && [ "$3" = "usages" ] && [ "$4" = "-i" ]; then
  echo 'Aicore Usage Rate : 88 %'
fi
""",
            ...
            env = os.environ.copy()
            env["PATH"] = f"{bin_dir}{os.pathsep}{env['PATH']}"
            env["AUTOPERF_MONITOR_PIDS"] = "201"
            env["AUTOPERF_MONITOR_RELATED_PIDS"] = "201"

            mem = subprocess.run(
                ["bash", str(ROOT / "scripts" / "run_npu_mem.sh"), "svc", "15", "0:0"],
                text=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                env=env,
            ...
            usage = subprocess.run(
                ["bash", str(ROOT / "scripts" / "run_npu_usage.sh"), "svc", "15", "0:0"],
                text=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                env=env,
            ...
            self.assertEqual(mem.stdout.strip(), "2048.00")
            self.assertEqual(usage.stdout.strip(), "88.00")
